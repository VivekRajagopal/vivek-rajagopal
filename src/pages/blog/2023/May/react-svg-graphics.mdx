...
the experiences is a 2D Graphics display, which displays all the engineering geometry and
annotations (dimensions, labels, etc). This graphics display allows the user to zoom and pan within
the view, and provides a kind of "CAD like" experience.
...
At the time these were good choices - they kept velocity high. As time's gone on, it's become more
difficult to ship new features because of the vendor lock-in with [PaperJS](http://paperjs.org/),
and the imperative updates to the view.
...
whatever geometric calculations were needed.
...
from the view and allow for a more declarative approach.

The final piece of the puzzle was the view. It was a question between SVG and Canvas. Canvas is a
bit more performant, but SVG is more declarative. In the end I decided to go with SVG to _index_ a
little more on the declarative side, and allow hooking into "browser-native" events like `onClick`
and `onMouseOver` of the SVG elements.
...
upwards in the y-axis is positive. In view space, (0, 0) is typically in the top-left corner, and
_downwards_ in the y-axis is positive.
...
an old laptop. Unfortunately, I couldn't compromise on the performance, espectially since I had a
...
I ended up using RxJS to try tidy up the imperative code a little, and trigger `setTransform` in the
subscription.
